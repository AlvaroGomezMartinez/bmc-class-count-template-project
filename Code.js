  ...
  var batchSize = parseInt(props.getProperty('CONSOLIDATE_BATCH_SIZE') || '15', 10);
    ...
    batch.forEach(function(item) {
      ...
      months.forEach(function(month) {
        ...
        var dataStartRow = month.toLowerCase().indexOf('projection') !== -1 ? 4 : 3;
        if (lr <= dataStartRow - 1) return; // headers/metadata only
        
        var values = sh.getRange(dataStartRow, 1, lr - dataStartRow + 1, lc).getValues();
    ...
    var appendSummary = [];
    var errorRows = [];
  var validValues = ['OHI', 'ED', 'AU', 'SI', 'CI', 'ID', 'LD', 'OTHER', 'Other', '2026-2027 Grade Level'];
  var validValuesUpper = validValues.map(function(v) { return v.toUpperCase(); });
  
  // Valid campus names from the error message + additional staff names found in data
  var validCampuses = ['Adams Hill', 'Aue', 'Behlau', 'Bernal', 'Blattman', 'Boldt', 'Boone', 'Brandeis', 'Brauchle', 'Braun Station', 'Brennan', 'Briscoe', 'Burke', 'Cable', 'Carlos Coon', 'Carnahan', 'Carson', 'Chumbley', 'Clark', 'Cody', 'Cole', 'Colonies North', 'Connally', 'Driggers', 'Ellison', 'Elrod', 'Evers', 'Fields', 'Fisher', 'Folks', 'Forester', 'Franklin', 'Galm', 'Garcia', 'Glass', 'Glenn', 'Glenoaks', 'Harlan', 'Hatchett', 'Henderson', 'Hobby', 'Hoffmann', 'Holmes', 'Holmgreen MS', 'Howsman', 'Jay', 'Jefferson', 'Jones', 'Jordan', 'Kallison', 'Krueger', 'Kuentz', 'Langley', 'Lewis', 'Lieck', 'Linton', 'Locke Hill', 'Los Reyes', 'Luna', 'Marshall', 'Martin', 'Mary Hull', 'May', 'McDermott', 'Mead', 'Meadow Village', 'Michael', 'Mireles', 'Murnin', 'NAHS', 'Neff', 'Northside Alternative MS', 'Northwest Crossing', 'O\'Connor', 'Oak Hills Terrace', 'Ott', 'Passmore', 'Pease', 'Powell', 'Raba', 'Rawlinson', 'Rayburn', 'Reed', 'Rhodes', 'Ross', 'Rudder', 'Scarborough', 'Scobee', 'Sotomayor', 'Steubing', 'Stevens', 'Stevenson', 'Stinson', 'Straus', 'Taft', 'Thornton', 'Timberwilde', 'Tomlinson', 'Vale', 'Valley Hi', 'Wanke', 'Ward', 'Warren', 'Wernli', 'WWT', 'Zachry', 'Mora', 'Nichols', 'Fernandez'];
  var validCampusesUpper = validCampuses.map(function(v) { return v.toUpperCase(); });
  
  Logger.log('Valid campuses array initialized with ' + validCampuses.length + ' items');
  Logger.log('Sample valid campuses (upper): ' + validCampusesUpper.slice(0, 10).join(', '));
    ...
    months.forEach(function(month) {
      ...
      if (rowsToAppend.length > 0) {
        ...
        for (var i = 0; i < rowsToAppend.length; i++) {
          ...
          if (valueDFinal && validCampusesUpper.indexOf(valueDFinalUpper) === -1) {
            finalValidationErrors.push('Row ' + (i + 1) + ': "' + valueDFinal + '" (upper: "' + valueDFinalUpper + '")');
            Logger.log('INVALID VALUE FOUND: "' + valueDFinal + '" -> "' + valueDFinalUpper + '"');
            Logger.log('Valid campuses array contains: ' + validCampusesUpper.slice(0, 10).join(', ') + '...');
            Logger.log('Index of "' + valueDFinalUpper + '" in valid array: ' + validCampusesUpper.indexOf(valueDFinalUpper));
          }
        }
        
        if (finalValidationErrors.length > 0) {
          Logger.log('FINAL VALIDATION FAILED for ' + month + ': ' + finalValidationErrors.join(', '));
          appendSummary.push(month + ': SKIPPED - Contains invalid campus names: ' + finalValidationErrors.join(', '));
        ...
        if (columnDValues.length > 0) {
          Logger.log('Writing to ' + month + ' column D: ' + columnDValues.join(', '));
        }
        ...
        for (var attempt = 1; attempt <= maxRetries && !success; attempt++) {
          try {
            ...
            if (attempt > 1) {
              Logger.log('Attempt ' + attempt + ' for ' + month + ' after ' + (retryDelay / 1000) + ' second delay...');
            ...
            if (attempt === 1) {
              try {
                ...
                var validationRange = dest.getRange(startRow, 1, rowsToAppend.length, lcDest);
                var validations = validationRange.getDataValidations();
                dataValidations = validations;
              } catch (validationError) {
                Logger.log('Could not get validation rules, proceeding without them: ' + validationError.toString());
                dataValidations = [];
              }
            }
            ...
            if (dataValidations.length > 0) {
              try {
                ...
                Logger.log('Could not clear validation rules: ' + clearError.toString());
              }
            }
            ...
            if (dataValidations.length > 0) {
              try {
                ...
                Logger.log('Could not restore validation rules: ' + restoreError.toString());
            ...
            Logger.log('Successfully wrote ' + rowsToAppend.length + ' rows to ' + month + ' (attempt ' + attempt + ')');
            
          } catch (e) {
            Logger.log('Attempt ' + attempt + ' failed for ' + month + ': ' + e.toString());
            
            if (attempt === maxRetries) {
              // Last attempt failed, log detailed error info
              Logger.log('All attempts failed for ' + month + '. Error details:');
              Logger.log('Error type: ' + e.name);
              Logger.log('Error message: ' + e.message);
              Logger.log('Rows to append: ' + rowsToAppend.length);
              Logger.log('Target range: ' + startRow + ':' + (startRow + rowsToAppend.length - 1));
              Logger.log('Sample row data: ' + JSON.stringify(rowsToAppend.slice(0, 2)));
              
              // Check if this is a rate limiting or service error
              var errorMsg = e.toString().toLowerCase();
              if (errorMsg.indexOf('service') !== -1 || 
                  errorMsg.indexOf('rate') !== -1 || 
                  errorMsg.indexOf('quota') !== -1 ||
                  errorMsg.indexOf('timeout') !== -1) {
                // This looks like a temporary service issue
                throw new Error('Google Sheets service temporarily unavailable for ' + month + '. Please try again in a few minutes. Original error: ' + e.toString());
              } else {
    ...
    if (errorRows.length > 0) {
      ...
      errorRows.forEach(function(e) {
        errorMsg += 'Campus: ' + e.campus + ', Month: ' + e.month + ', Row: ' + e.row + ', Column: ' + e.column + ', Value: "' + e.value + '", Error: ' + e.error + '\n';
      });
      errorMsg += '\n';
    }
    ...
    if (!isAutoMode) {
      SpreadsheetApp.getUi().alert(errorMsg + 'Level ' + level + ' batch complete.\nProcessed campuses: ' + batch.length + '\n' + (appendSummary.length ? appendSummary.join(', ') : 'No data this run') + '\nProgress: ' + endIndex + ' / ' + levelRows.length + (done ? ' (DONE)' : ''));
    } else {
      // For auto mode, just log the completion
      Logger.log('Auto mode: Level ' + level + ' batch complete. Processed campuses: ' + batch.length + '. Progress: ' + endIndex + ' / ' + levelRows.length + (done ? ' (DONE)' : ''));
    }
    
    if (isAutoMode && !done) {
      // Schedule the next batch (no UI in auto mode)
      Logger.log('Auto mode: Scheduling next batch for ' + level + '. Progress: ' + endIndex + ' / ' + levelRows.length);
    ...
    Logger.log('Error in consolidateLevelNextBatch_ for ' + level + ': ' + batchError.toString());
    ...
    if (isAutoMode) {
      ...
      if (errorMsg.indexOf('service') !== -1 || 
        ...
        Logger.log('Service error detected, scheduling retry for ' + level + ' with longer delay...');
        ...
        props.deleteProperty('AUTO_MODE_' + level);
        cleanupTriggers_(level);
        Logger.log('Serious error in auto mode for ' + level + ', stopping automation: ' + batchError.toString());
...
function stopAutoProcessing() {
  ...
  SpreadsheetApp.getUi().alert('🛑 Automated processing stopped.\nAll triggers have been cleaned up.');
}
...
function autoConsolidateLevel_(level) {
  ...
  if (props.getProperty('AUTO_MODE_' + level) === 'true') {
    SpreadsheetApp.getUi().alert('⚠️ Automated processing for ' + level + ' is already running.\nUse "Stop Auto Processing" to cancel first.');
    return;
  }
  ...
  if (!props.getProperty('AUTO_ALL_LEVELS')) {
    SpreadsheetApp.getUi().alert('🚀 Starting automated processing for ' + level + ' level.\n\nProcessing will continue automatically overnight.\nUse "Stop Auto Processing" to cancel at any time.');
  }
}
...
function scheduleNextBatch_(level, isRetry) {
  try {
    ...
    Logger.log('Scheduled next batch for ' + level + ' in ' + delayMinutes + ' minutes. Trigger ID: ' + trigger.getUniqueId());
  } catch (error) {
    Logger.log('Error scheduling next batch for ' + level + ': ' + error.toString());
...
function cleanupTriggers_(level) {
  ...
  if (triggerId) {
    try {
      ...
      Logger.log('Error cleaning up trigger for ' + level + ': ' + error.toString());
    }
  ...
  for (var i = 0; i < triggers.length; i++) {
    ...
    if (handlerFunction === 'autoContinueBatch_' + level) {
      try {
        ScriptApp.deleteTrigger(triggers[i]);
        Logger.log('Cleaned up orphaned trigger for ' + level + ': ' + triggers[i].getUniqueId());
      } catch (error) {
...
function sendCompletionEmail_(level, type) {
  try {
    ...
    if (type === 'sequence') {
      ...
      body = 'Great news! The automated BMC consolidation process has successfully completed all levels.\n\n' +
             '✅ ES Level: Complete\n' +
             '✅ MS Level: Complete\n' +
             '✅ HS Level: Complete\n\n' +
             'Your consolidated report is ready for review.\n\n' +
             'Spreadsheet: ' + spreadsheetName + '\n' +
             'View Report: ' + spreadsheetUrl + '\n\n' +
             'Generated automatically by BMC Class Count Template Project';
    } else {
      subject = '✅ BMC Consolidation Complete - ' + level + ' Level';
      body = 'The automated BMC consolidation process has successfully completed for the ' + level + ' level.\n\n' +
...
function sendErrorEmail_(level, errorMessage) {
  try {
    ...
    var body = 'The automated BMC consolidation process encountered an error and has been stopped.\n\n' +
               'Level: ' + level + '\n' +
               'Error: ' + errorMessage + '\n\n' +
               'You can resume the process manually using the "Next Batch ' + level + '" option in the BMC menu.\n\n' +
               'Spreadsheet: ' + spreadsheetName + '\n' +
               'Access Spreadsheet: ' + spreadsheetUrl + '\n\n' +
               'Generated automatically by BMC Class Count Template Project';
